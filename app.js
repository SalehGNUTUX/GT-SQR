...
const S = {
  ...
  bgVidItems: [], // [{file, vid, name, dur, url, hidden, trimStart, trimEnd, transition}] — playlist لخلفية الفيديو
...
function drawWatermark(ctx, W, H) {
  ...
  const yOff = (parseFloat(gv("wm-y-offset")) || 0) / 100;
...
function applyBgVidTrim() {
  ...
  // v1.2 — أُزيل مُراقِب timeupdate (كان يَقفِل المَقطع في وَضع playlist).
...
// v1.2 — الحَذف الأَصليّ (بدون Undo history)
...
function startPlayer() {
  ...
  // v3.3.6 — في وضع recvid: لا تُشغّل bgAudio
  ...
  // v1.2 — شَغِّل bgVidNext إن كان في مَنتَصَف الـcrossfade
...
function restartAll() {
  ...
  if (Array.isArray(S.bgVidItems)) {
    ...
    // v1.2 — أَعِد التَنشيط على أَوّل مَقطع
    if (S.bgVidItems[0]) {
...
async function startExport(type) {
  ...
  if (typeof startWebExportV2 === "function" && typeof isWebCodecsSupported === "function" && isWebCodecsSupported()) {
    try {
      ...
      let bgBuffer = null;
      // v3.3.6 — recvid → استخدم صوت فيديو التلاوة بدل bgAudio
...
async function restoreAssetFromDataURL(asset) {
  ...
  if (asset.key === "logo") {
    ...
    if (typeof addBgVidItem === "function") {
      // v1.2 — انتَظِر onloadeddata ثمّ طَبِّق الإعدادات مُباشَرَةً على item المُعاد
